import com.joohnq.muscle_management.domain.entity.*
...
class SignUpUseCaseTest {
    ...
    fun `should return success when email and password are valid`() = runTest {
        coEvery { repository.signUp("user@example.com", "123456") } returns Unit

        val result = useCase("user@example.com", "123456")

        assertTrue(result.isSuccess)
        coVerify(exactly = 1) { repository.signUp("user@example.com", "123456") }
    }
    ...
    fun `should fail when repository throws exception`() = runTest {
        ...
        coEvery { repository.signUp(any(), any()) } throws runtimeException
